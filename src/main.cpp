...
#if defined(NDEBUG)
# error "Asgardiacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd512f57b9852d2ba49d36e0ec353ef818c6d1a5d782dcfe8841125971bb44e59");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Asgardiacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Asgardiacoin Signed Message:\n";
...
{
    // Asgardiacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Asgardiacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Asgardiacoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Asgardiacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Asgardiacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Asgardiacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x88d4a22709fe178971c25abc23b9f3b9b154c09381e5aebd947d5860ec3ebf8e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1516538908;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085722560;

        if (fTestNet)
        {
            block.nTime    = 1516538869;
            block.nNonce   = 385882717;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2befcbed87805a5a823f4b0c599737aa9c54af6104d693afd0006a5459265536"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Asgardiacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AsgardiacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AsgardiacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AsgardiacoinMiner(CWallet *pwallet)
{
    printf("AsgardiacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("asgardiacoin-miner");
    ...
    try { loop {
        ...
        printf("Running AsgardiacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AsgardiacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AsgardiacoinMiner, pwallet));
}
